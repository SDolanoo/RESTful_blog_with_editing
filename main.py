...
def get_all_posts():
    ...
    posts = []
    query = db.session.execute(db.select(BlogPost).order_by(BlogPost.id)).scalars()
    for q in query:
        posts.append(q)
...
def new_post():
    ...
    return render_template("make-post.html", form=form, editing=False)
